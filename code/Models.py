...
class BaseModel:
    ...
    def compile_and_fit(self, xy_train, xy_valid, fit_args):
        ...
        early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=fit_args['early_stopping'])
        fit_args['callbacks'] = [early_stopping]
        del fit_args['early_stopping']
        history = self.model.fit(x_train, y_train, validation_data=(x_valid, y_valid), **fit_args)
