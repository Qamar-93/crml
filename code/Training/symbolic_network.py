...
class SymbolicLayerL0(SymbolicLayer):
    ...
    def sample_u(self, shape, reuse_u=False):
        """Uniform random numbers for concrete distribution"""
        device = self.qz_log_alpha.device  # Use qz_log_alpha’s device
        if self.eps is None or not reuse_u:
            self.eps = torch.rand(size=shape, device=device) * (1 - 2 * self.epsilon) + self.epsilon
        else:
    ...
    def sample_weights(self, reuse_u=False):
        ...
        mask = mask.to(self.W.device)  # Ensure mask is on the same device
    ...
    def forward(self, x, sample=True, reuse_u=False):
        ...
        self.qz_log_alpha.data = self.qz_log_alpha.data.to(self.W.device)

        # Ensure the input dimension of x matches the expected input dimension
        if x.shape[1] != self.in_dim:
            
            raise ValueError(f"Expected input dimension {self.in_dim}, but got {x.shape[1]}")
...
class SymbolicNetL0(nn.Module):
    ...
    def __init__(self, symbolic_depth, in_dim=1, funcs=None, initial_weights=None, init_stddev=0.1, seed=42):
        ...
        if initial_weights is not None:
            ...
            layers = [SymbolicLayerL0(funcs=funcs, init_stddev=init_stddev[i], in_dim=layer_in_dim[i])
                      for i in range(self.depth)]
            # Use layers[-1].n_funcs directly
            last_layer_n_funcs = layers[-1].n_funcs
            self.output_weight = nn.Parameter(
                torch.rand(size=(last_layer_n_funcs, 1)) * 2
            )

        # Now define the sequential
        self.hidden_layers = nn.Sequential(*layers)
    ...
    def forward(self, input, sample=True, reuse_u=False):
        ...
        self.output_weight.data = self.output_weight.data.to(h.device)
    ...
    def predict(self, input, sample=True, reuse_u=False):
        """Perform a forward pass and return the output."""
        self.eval()  # Set the model to evaluation mode
        with torch.no_grad():
            return self.forward(input, sample=sample, reuse_u=reuse_u)
