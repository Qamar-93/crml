...
def extract_g(y_noisy, y_clean):
        # print("1 y_noisy shape:", y_noisy.shape, "y_clean shape:", y_clean.shape)
        y_noisy_min = torch.min(y_noisy, dim=0).values
        y_noisy_max = torch.max(y_noisy, dim=0).values
        
        y_noisy_min = y_noisy_min.to(torch.float64)
        y_noisy_max = y_noisy_max.to(torch.float64)
        
        y_noisy_min = y_noisy_min.to(y_noisy.device)
        y_noisy_max = y_noisy_max.to(y_noisy.device)
        y_clean = y_clean.to(y_noisy.device)
        # print("2 y_noisy_min shape:", y_noisy_min.shape, "y_noisy_max shape:", y_noisy_max.shape)
        ...
        # print("3 y_dist_min shape:", y_dist_min.shape, "y_dist_max shape:", y_dist_max.shape)
        # now gy is the maximum of the two distances: dist_min and dist_max
        gy = torch.maximum(y_dist_min, y_dist_max)
        # print("4 gy shape:", gy.shape)
        # Find the index of the minimum and maximum distances by masking; then use the index to get the corresponding y_noisy value
        ...
        # print("5 g_points shape:", g_points.shape)
        return g_points

def calculate_penalty(outputs, y_clean, x_noisy_torch, net, baseline_dist):
        # Forward pass for y_noisy
        y_noisy_pred = torch.stack([net(x.to(outputs.device)) for x in x_noisy_torch])
        gy = extract_g(y_noisy_pred, y_clean)

        y_clean_scaled, gy_scaled = rescale_vectors(y_clean, gy)
        
        gy_y_dist = torch.sqrt(torch.sum((gy_scaled - y_clean_scaled) ** 2))
        ratio = gy_y_dist / baseline_dist  # Assuming bl_ratio is 40.0
...
##############################################
def custom_train_function(configs, res_folder, loss_function="mse", bl_dist=1):
    """
    ...
    for i in range(models_no):    
        ...
        for epoch in range(epochs):
            ...
            mse_loss = criterion(outputs, y_train_torch)
            panalty = calculate_penalty(outputs, y_clean_torch, x_noisy_torch, net, bl_dist)
            
            # mse_loss = criterion(y_clean_torch, outputs, epoch)
            
            reg_loss = net.get_loss()
            loss = mse_loss + 5e-3 * reg_loss
            loss = loss * panalty
...
def main(res_folder, json_path, loss_function, noise_type, epsilon=0.5):
    ...
    with open(configs) as f:
        configs = json.load(f)
    eq_res_folder = f'{res_folder}/{json_path.split("/")[-1].split(".")[0]}'
    ...
    # exit()
    models_folder = f"{eq_res_folder}/models"
    print(configs)
    x_len = configs["num_samples"]
    num_noises = 20
    distribution = "normal"
    ...
    load_and_test(models_folder, [0], equation_str, noise_model, input_features, x_len)
