...
def main(res_folder, json_path, loss_fuction, noise_type, epsilon=0.5):
    ...
    for config in configs["models"]:
        ...
        if best_model is not None:
            ...
            evaluate_and_plot(trainer.model, best_history, xy_test, f"{plots_folder}")
####### evaluate model robustness
        ...
        for idx, model in enumerate(models):
            ...
            print(y_clean_orig.shape)
            y_pred = model.predict(x_clean_orig)
            
            # plot the output of the model
            import matplotlib.pyplot as plt
            plt.clf()
            plt.plot(y_clean_orig, label="y_clean")
            plt.plot(y_pred, label="y_pred")
            plt.legend()
            plt.savefig(f"{model_i_res_folder}/y_pred.png")
            print("mse of y_clean and y_pred", np.mean((y_clean_orig - y_pred)**2))
            exit()
...
if __name__ == '__main__':
    ...
    for json_file in json_files:
        ...
        noise_type = "normal"
        loss_fuction = "mse"
        res_folder = f"./results_loading_{os.path.splitext(json_file)[0]}/loss_{loss_fuction}/{noise_type}/non-dp/"
